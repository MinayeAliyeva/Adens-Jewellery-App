...
const FavoriteIcon = () => {
    ...
    const { data: favoriteData }= useGetFavoriteByUserIdQuery({userId: userData?._id ?? ""}, { skip: !userData?._id }); 
    const count= useSelector(getUserFavoriteProductCountSelector); 
   const favoriteDataCount = count > 0 ?  count : (favoriteData?.products?.length || 0);
